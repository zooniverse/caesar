...
RSpec.describe UserRulesController, type: :controller do
  ...
  context 'as a permissioned user' do
    ...
    describe '#destroy' do
      it 'lets a user delete user_rules if they own the workflow' do
        ur2 = create :user_rule, workflow: workflow
        response = delete :destroy, params: { id: ur2.id, workflow_id: workflow.id }, format: :json

        expect(response.status).to eq(204)
        expect(UserRule.find_by_id(ur2.id)).to be(nil)
      end

      it 'does not let a user delete user_rules if they do not own the workflow' do
        other_workflow = create :workflow, project_id: workflow.project_id + 1
        ur2 = create :user_rule, workflow: other_workflow
        response = delete :destroy, params: { id: ur2.id, workflow_id: workflow.id }, format: :json

        expect(response.status).to eq(404)
        expect(UserRule.find_by_id(ur2.id)).not_to be(nil)
      end
    end
  end
