...
describe CsvExporter do
  ...
  describe 'working with Extracts' do

    let(:exporter){ described_class.new(:extracts) }
    let(:sample){
      Extract.new(
        :extractor_id => "x",
    ...
    before do
      if(File.exists?"tmp/extracts_#{workflow_id}.csv")
    ...
    it 'should create the right file' do
      ...
      expect(File.exists?("tmp/extracts_#{workflow_id}.csv")).to be(true)
    end
  ...
  describe 'working with Reductions' do
    let(:exporter){ described_class.new(:reductions) }
    let(:sample){
      Reduction.new(
        :reducer_id => "x",
        :workflow_id => workflow_id,
    ...
    before do
      if(File.exists?"tmp/reductions_#{workflow_id}.csv")
    ...
    it 'should create the right file' do
      exporter.dump(workflow_id)
      expect(File.exists?("tmp/reductions_#{workflow_id}.csv")).to be(true)
    end
