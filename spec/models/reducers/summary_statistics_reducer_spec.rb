...
describe Reducers::SummaryStatisticsReducer do
  ...
  describe '#configuration' do
    it 'requires configuration fields' do
      r1 = described_class.new(reducible_id: workflow.id, config: {})
    ...
    it 'requires a valid field to summarize' do
      r1 = described_class.new(reducible_id: workflow.id, config: {"summarize_field" => "simple_field"})
      r2 = described_class.new(reducible_id: workflow.id, config: {"summarize_field" => "complex.field"})
      r1.validate
      r2.validate
      ...
      r3 = described_class.new(reducible_id: workflow.id, config: {"summarize_field" => "bad."})
      r3.validate
    ...
    it 'requires a valid list of operations' do
      r1 = described_class.new(reducible_id: workflow.id, config: {"operations" => 0})
      r1.validate
      expect(r1.errors[:operations]).to be_present

      r2 = described_class.new(reducible_id: workflow.id, config: {"operations" => "blah"})
      r2.validate
      expect(r2.errors[:operations]).to be_present

      r3 = described_class.new(reducible_id: workflow.id, config: {"operations" => "sum"})
      r3.validate
      expect(r3.errors[:operations]).not_to be_present

      r4 = described_class.new(reducible_id: workflow.id, config: {"operations" => []})
      r4.validate
      expect(r4.errors[:operations]).to be_present

      r5 = described_class.new(reducible_id: workflow.id, config: {"operations" => [0]})
      r5.validate
      expect(r5.errors[:operations]).to be_present

      r6 = described_class.new(reducible_id: workflow.id, config: {"operations" => ["blah"]})
      r6.validate
      expect(r6.errors[:operations]).to be_present

      r7 = described_class.new(reducible_id: workflow.id, config: {"operations" => ["sum"]})
      r7.validate
      expect(r7.errors[:operations]).not_to be_present

      r8 = described_class.new(reducible_id: workflow.id, config: {"operations" => ["sum", 0]})
      r8.validate
      expect(r8.errors[:operations]).to be_present

      r9 = described_class.new(reducible_id: workflow.id, config: {"operations" => ["sum", "blah"]})
      r9.validate
      expect(r9.errors[:operations]).to be_present

      r10 = described_class.new(reducible_id: workflow.id, config: {"operations" => ["sum", "mean"]})
      r10.validate
    ...
    it 'reads the config fields correctly' do
      ...
      r3 = described_class.new(reducible_id: workflow.id, config: {"operations" => ["sum", "mean"]})
      expect(r3.send(:operations)).to eq(["sum", "mean"])

      r4 = described_class.new(reducible_id: workflow.id, config: {"operations" => "sum"})
      expect(r4.send(:operations)).to eq(["sum"])

      r5 = described_class.new(reducible_id: workflow.id, config: {"operations" => ["sum","sum"]})
